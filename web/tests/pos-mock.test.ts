...
describe('mockPosAdapter', () => {
  ...
  it('returns the Falls Church pilot location and bilingual synthetic menu', async () => {
    ...
    expect(locations).toEqual([
      { id: 'lantern-house-falls-church', name: 'Lantern House Falls Church' },
    ]);
  ...
  it('accepts a synthetic order without inventing an unverified price', async () => {
    const result = await mockPosAdapter.createOrder('lantern-house-falls-church', {
      fulfillment: 'pickup',
      customerName: 'Synthetic Customer',
      lines: [
        {
          menuItemId: 'mock-pho-tai',
          quantity: 2,
          modifierOptionIds: ['mock-size-regular', 'mock-no-onion'],
        },
      ],
    });
